...
(function (global, factory) {
  (typeof exports === "undefined" ? "undefined" : _typeof(exports)) === 'object' && typeof module !== 'undefined' ? factory(exports) : typeof define === 'function' && define.amd ? define(['exports'], factory) : (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.WrapText_unicode = {}));
  ...
  var ensureDataReady = function ensureDataReady() {
    ...
    for (var lineBreakClass in data) {
      if (data.hasOwnProperty(lineBreakClass)) {
        ...
        for (var _i = rangeCompacts.length - 1; _i >= 0; _i -= 2) {
          var start = rangeCompacts[_i] + nextEnd;
          nextEnd = rangeCompacts[_i - 1] + start; // end === nextEnd

          for (var point = nextEnd + 1; --point >= start;) {
            pointSet.add(point);
          }
        }
  ...
  var cc = function cc(charCode, lineBreakClass) {
    return data[lineBreakClass].has(charCode);
  };

  var mc = function mc(charCode, lineBreakClassList) {
    for (var i = 0; i < lineBreakClassList.length; i++) {
  ...
  var charLoaderFn = function charLoaderFn(loadedChars, _char, collapsesSpaces) {
    ...
    if (loadedChars.length > 0 && !mc(loadedChars[loadedChars.length - 1].charCodeAt(0), ['BK', 'CR', 'LF', 'NL', 'SP', 'ZW']) && mc(_char.charCodeAt(0), ['CM', 'ZWJ'])) {
      loadedChars[loadedChars.length - 1] += _char;
      return;
    }
  ...
  var lineBreakOpportunityTestFn = function lineBreakOpportunityTestFn(charProvider, indexBefore, trimsLines) {
    ...
    var beforeBefore = charProvider.get_(indexBefore - 1).charCodeAt(0);
    var before = charProvider.get_(indexBefore).charCodeAt(0);
    var after = charProvider.get_(indexBefore + 1).charCodeAt(0); // If we have a character and zero, one or many following spaces
    // Then we call that character beforeSP

    var beforeSP = before; // indexBefore

    if (cc(beforeSP, 'SP')) {
      beforeSP = beforeBefore; // indexBefore - 1

      if (cc(beforeSP, 'SP')) {
        for (var i = indexBefore - 2; cc(beforeSP, 'SP'); i--) {
          beforeSP = charProvider.get_(i).charCodeAt(0); // beforeSP will be NaN if there is no character left
    ...
    if (mc(before, ['AL', 'HL']) && cc(after, 'NU') || cc(before, 'NU') && mc(after, ['AL', 'HL'])) {
      return false;
    } // LB23a


    if (cc(before, 'PR') && mc(after, ['ID', 'EB', 'EM']) || mc(before, ['ID', 'EB', 'EM']) && cc(after, 'PO')) {
      return false;
    } // LB24


    if (mc(before, ['PR', 'PO']) && mc(after, ['AL', 'HL']) || mc(before, ['AL', 'HL']) && mc(after, ['PR', 'PO'])) {
      return false;
    } // LB25


    if (mc(before, ['CL', 'CP', 'NU']) && mc(after, ['PO', 'PR']) || mc(before, ['PO', 'PR']) && cc(after, 'OP') || mc(before, ['PO', 'PR', 'HY', 'IS', 'NU', 'SY']) && cc(after, 'NU')) {
      return false;
    } // LB26


    if (cc(before, 'JL') && mc(after, ['JL', 'JV', 'H2', 'H3']) || mc(before, ['JV', 'H2']) && mc(after, ['JV', 'JT']) || mc(before, ['JT', 'H3']) && cc(after, 'JT')) {
      return false;
    } // LB27


    if (mc(before, ['JL', 'JV', 'JT', 'H2', 'H3']) && mc(after, ['IN', 'PO']) || cc(before, 'PR') && mc(after, ['JL', 'JV', 'JT', 'H2', 'H3'])) {
      return false;
    } // LB28
    ...
    if (mc(before, ['AL', 'HL', 'NU']) && cc(after, 'OP') || cc(before, 'CP') && mc(after, ['AL', 'HL', 'NU'])) {
      return false;
    } // LB30a: Skip
    // LB30b
  ...
  var spacesAsTab; // IMPORTANT!!!
  // Always set the tab size before using other functions

  var setTabSize = function setTabSize(value) {
  ...
  var tw = function tw(chars) {
    ...
    for (var i = 0; i < chars.length; i++) {
      var _char2 = chars[i];
      textToMeasure += _char2 === '\t' ? spacesAsTab : _char2;
    }
  ...
  var lineFittingFn = function lineFittingFn(lineChars, charProvider, offset, currentMaxWidth) {
    while (charProvider.has_(offset + lineChars.length) && tw(lineChars) < currentMaxWidth) {
    ...
    if (lineChars.length === 0) {
      ...
      while (lineChars.length > 1 && tw(lineChars) > currentMaxWidth) {
        lineChars.length -= 1;
      }
    }
  };
  ...
  var etcLineFittingFn = function etcLineFittingFn(lastLineChars, etc, currentMaxWidth) {
    while (lastLineChars.length > 0 && tw(lastLineChars.concat(etc)) > currentMaxWidth) {
